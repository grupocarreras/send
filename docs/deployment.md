## Requirements
This document describes how to do a full deployment of Carreras Send on your own Linux server. You will need:
...
* First, remove that htdocs folder - we will replace it with Carreras Send's version now
...
## Running
To have a permanently running version of Carreras Send as a background process:
...
Now the Carreras Send backend should be running on port 1443. You can check with:
* telnet localhost 1443

## Reverse Proxy
Of course, we don't want to expose the service on port 1443. Instead we want our normal webserver to forward all requests to Carreras Send ("Reverse proxy").
